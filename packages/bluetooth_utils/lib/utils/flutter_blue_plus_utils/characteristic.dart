...
extension CharacteristicFlutterBluePlus on FlutterBluePlus {
  ...
  static Stream<BluetoothCharacteristic> get onCharacteristicReceived => FlutterBluePlusPlatform.instance.onCharacteristicReceived
    .map((p) => p.characteristic)
    .where((p) => p != null)
    .map((p) => p!);

  static Stream<BluetoothCharacteristic> get onCharacteristicWritten => FlutterBluePlusPlatform.instance.onCharacteristicWritten
    .map((p) => p.characteristic)
    .where((p) => p != null)
    .map((p) => p!);

  static Stream<BluetoothCharacteristic> get onCharacteristicLastValueStream => _mergeStreams([
    onCharacteristicReceived,
    onCharacteristicWritten,
  ]);

  static Stream<BluetoothCharacteristic> get onNotifyValueChanged => _mergeStreams([
...
extension ValueBluetoothCharacteristic on BluetoothCharacteristic {
  ...
  Stream<List<int>> get onValueWritten => FlutterBluePlusPlatform.instance.onCharacteristicWritten
  ...
  Stream<bool> get onNotifyValueChanged => CharacteristicFlutterBluePlus.onNotifyValueChanged
    .where((p) => p.remoteId == remoteId)
    .where((p) => p.serviceUuid == serviceUuid)
    .where((p) => p.characteristicUuid == characteristicUuid)
    .where((p) => p.primaryServiceUuid == primaryServiceUuid)
