...
extension DescriptorFlutterBluePlus on FlutterBluePlus {
  ...
  static Stream<BluetoothDescriptor> get onDescriptorReceived => FlutterBluePlusPlatform.instance.onDescriptorRead
    .map((p) => p.descriptor)
    .where((p) => p != null)
    .map((p) => p!);

  static Stream<BluetoothDescriptor> get onDescriptorWritten => FlutterBluePlusPlatform.instance.onDescriptorWritten
    .map((p) => p.descriptor)
    .where((p) => p != null)
    .map((p) => p!);

  static Stream<BluetoothDescriptor> get onDescriptorLastValueStream => _mergeStreams([
    onDescriptorReceived,
    onDescriptorWritten,
  ]);
...
extension ValueBluetoothDescriptor on BluetoothDescriptor {
  ...
  Stream<List<int>> get onValueWritten => FlutterBluePlusPlatform.instance.onDescriptorWritten
