...
extension BluetoothServicesFlutterBluePlus on FlutterBluePlus {
  static Stream<BluetoothDevice> get onDiscoveredServices => FlutterBluePlusPlatform.instance.onDiscoveredServices
    .map((p) => BluetoothDevice.fromId(p.remoteId.str));

  static Stream<BluetoothDevice> get onServicesReset => FlutterBluePlusPlatform.instance.onServicesReset
    .map((p) => BluetoothDevice.fromId(p.remoteId.str));
...
extension BluetoothServicesBluetoothDevice on BluetoothDevice {
  Stream<List<BluetoothService>> get onDiscoveredServices => FlutterBluePlusPlatform.instance.onDiscoveredServices
    .where((p) => p.remoteId == remoteId)
    .where((p) => p.success)
    .map((p) => p.services.map((p) => BluetoothService.fromProto(p)).toList());

  Stream<void> get onServicesReset => FlutterBluePlusPlatform.instance.onServicesReset
    .where((p) => p.remoteId == remoteId)
    .map((_) => []);
